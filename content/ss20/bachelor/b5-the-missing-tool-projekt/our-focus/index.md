...
Another use case is that the professor asks the students to split into groups based on the topics that just came up while brainstorming. In this case, each student can simply select the topic that seems most interesting to him/her and directly jump to another board from the existing card on the board. All students that click on the same link are then on a new board where they can immediately start working on their ideas.
{{</section>}}
...
Do you fancy finding out about the technologies we used in our project? Feel free to read on:
